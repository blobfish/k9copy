 ...
 */
#define DVDUDFCacheLevel(a,b) ___DVDUDFCacheLevel(a,b)
int ___DVDUDFCacheLevel(dvd_reader_t *device, int level)
{
...
void *___GetUDFCacheHandle(dvd_reader_t *device)
{
  struct dvd_reader_s *dev = (struct dvd_reader_s *)device;
...
void ___SetUDFCacheHandle(dvd_reader_t *device, void *cache)
{
  struct dvd_reader_s *dev = (struct dvd_reader_s *)device;
...
void *___GetAlignHandle(dvd_reader_t *device)
{
  struct dvd_reader_s *dev = (struct dvd_reader_s *)device;
...
void ___SetAlignHandle(dvd_reader_t *device, void *align)
{
  struct dvd_reader_s *dev = (struct dvd_reader_s *)device;
...
#define initAllCSSKeys(a) ___initAllCSSKeys(a)
static int ___initAllCSSKeys( dvd_reader_t *dvd )
{
 ...
 */
#define DVDOpenImageFile(a,b) ___DVDOpenImageFile(a,b)
static dvd_reader_t *___DVDOpenImageFile( const char *location, int have_css )
{
  dvd_reader_t *dvd;
  dvd_input_t dev;
  int verbose;
...
#define DVDOpenPath(a) ___DVDOpenPath(a)
static dvd_reader_t *___DVDOpenPath( const char *path_root )
{
  dvd_reader_t *dvd;
...
dvd_reader_t *___DVDOpen( const char *path )
{
...
void ___DVDClose( dvd_reader_t *dvd )
{
  if( dvd ) {
...
void ___DVDInit(void)
{
  dvdinput_setup();
}

void ___DVDFinish(void)
{
  dvdinput_free();
}
 ...
 */
#define DVDOpenFileUDF(a,b) ___DVDOpenFileUDF(a,b)
static dvd_file_t *___DVDOpenFileUDF( dvd_reader_t *dvd, char *filename )
{
  uint32_t start, len;
  dvd_file_t *dvd_file;
 ...
 */
#define findDirFile(a,b,c) ___findDirFile(a,b,c)
static int ___findDirFile( const char *path, const char *file, char *filename )
{
  DIR *dir;
  struct dirent *ent;
...
#define findDVDFile(a,b,c) ___findDVDFile(a,b,c)
static int ___findDVDFile( dvd_reader_t *dvd, const char *file, char *filename )
{
 ...
 */
#define DVDOpenFilePath(a,b) ___DVDOpenFilePath(a,b)
static dvd_file_t *___DVDOpenFilePath( dvd_reader_t *dvd, char *filename )
{
...
#define DVDOpenVOBUDF(a,b,c) ___DVDOpenVOBUDF(a,b,c)
static dvd_file_t *___DVDOpenVOBUDF( dvd_reader_t *dvd, int title, int menu )
{
  char filename[ MAX_UDF_FILE_NAME_LEN ];
  uint32_t start, len;
  dvd_file_t *dvd_file;
...
#define DVDOpenVOBPath(a,b,c) ___DVDOpenVOBPath(a,b,c)
static dvd_file_t *___DVDOpenVOBPath( dvd_reader_t *dvd, int title, int menu )
{
  char filename[ MAX_UDF_FILE_NAME_LEN ];
  char full_path[ PATH_MAX + 1 ];
  struct stat fileinfo;
...
dvd_file_t *___DVDOpenFile( dvd_reader_t *dvd, int titlenum,
...
void ___DVDCloseFile( dvd_file_t *dvd_file )
{
  int i;
...
#define DVDFileStatVOBUDF(a,b,c,d) ___DVDFileStatVOBUDF(a,b,c,d)
static int ___DVDFileStatVOBUDF(dvd_reader_t *dvd, int title,
                             int menu, dvd_stat_t *statbuf)
{
  ...
  if( title == 0 ) {
    ...
    sprintf( filename, "/VIDESetUDFCacheHandleO_TS/VTS_%02d_%d.VOB", title, menu ? 0 : 1 );
  }
...
#define DVDFileStatVOBPath(a,b,c,d) ___DVDFileStatVOBPath(a,b,c,d)
static int ___DVDFileStatVOBPath( dvd_reader_t *dvd, int title,
                                       int menu, dvd_stat_t *statbuf )
{
  char filename[ MAX_UDF_FILE_NAME_LEN ];
  char full_path[ PATH_MAX + 1 ];
...
int ___DVDFileStat(dvd_reader_t *dvd, int titlenum,
...
int ___UDFReadBlocksRaw( dvd_reader_t *device, uint32_t lb_number,
 ...
 */
#define DVDReadBlocksUDF(a,b,c,d,e) ___DVDReadBlocksUDF(a,b,c,d,e)
static int ___DVDReadBlocksUDF( dvd_file_t *dvd_file, uint32_t offset,
                             size_t block_count, unsigned char *data,
                             int encrypted )
{
 ...
 */
#define DVDReadBlocksPath(a,b,c,d,e) ___DVDReadBlocksPath(a,b,c,d,e)
static int ___DVDReadBlocksPath( dvd_file_t *dvd_file, unsigned int offset,
                              size_t block_count, unsigned char *data,
                              int encrypted )
{
  int i;
 ...
 */
ssize_t ___DVDReadBlocks( dvd_file_t *dvd_file, int offset,
                       size_t block_count, unsigned char *data )
{
  int ret;
...
int ___DVDFileSeek( dvd_file_t *dvd_file, int offset )
{
  /* Check arguments. */
...
ssize_t ___DVDReadBytes( dvd_file_t *dvd_file, void *data, size_t byte_size )
{
...
ssize_t ___DVDFileSize( dvd_file_t *dvd_file )
{
  /* Check arguments. */
  if( dvd_file == NULL )
    return -1;
...
int ___DVDDiscID( dvd_reader_t *dvd, unsigned char *discid )
{
  struct md5_ctx ctx;
  int title;
  int nr_of_files = 0;
...
int ___DVDISOVolumeInfo( dvd_reader_t *dvd,
...
int ___DVDUDFVolumeInfo( dvd_reader_t *dvd,
                      char *volid, unsigned int volid_size,
                      unsigned char *volsetid, unsigned int volsetid_size )
{
  int ret;
