...
static int ___ifoRead_VMG(ifo_handle_t *ifofile);
#define ifoRead_VMG(a) ___ifoRead_VMG(a)
static int ___ifoRead_VTS(ifo_handle_t *ifofile);
#define ifoRead_VTS(a) ___ifoRead_VTS(a)
static int ___ifoRead_PGC(ifo_handle_t *ifofile, pgc_t *pgc, unsigned int offset);
#define ifoRead_PGC(a,b,c) ___ifoRead_PGC(a,b,c)
static int ___ifoRead_PGC_COMMAND_TBL(ifo_handle_t *ifofile,
                                   pgc_command_tbl_t *cmd_tbl, 
                                   unsigned int offset);
#define ifoRead_PGC_COMMAND_TBL(a,b,c) ___ifoRead_PGC_COMMAND_TBL(a,b,c)
static int ___ifoRead_PGC_PROGRAM_MAP(ifo_handle_t *ifofile,
                                   pgc_program_map_t *program_map, 
                                   unsigned int nr, unsigned int offset);
#define ifoRead_PGC_PROGRAM_MAP(a,b,c,d) ___ifoRead_PGC_PROGRAM_MAP(a,b,c,d)
static int ___ifoRead_CELL_PLAYBACK_TBL(ifo_handle_t *ifofile,
                                     cell_playback_t *cell_playback, 
                                     unsigned int nr, unsigned int offset);
#define ifoRead_CELL_PLAYBACK_TBL(a,b,c,d) ___ifoRead_CELL_PLAYBACK_TBL(a,b,c,d)
static int ___ifoRead_CELL_POSITION_TBL(ifo_handle_t *ifofile,
                                     cell_position_t *cell_position, 
                                     unsigned int nr, unsigned int offset);
#define ifoRead_CELL_POSITION_TBL(a,b,c,d) ___ifoRead_CELL_POSITION_TBL(a,b,c,d)
static int ___ifoRead_VTS_ATTRIBUTES(ifo_handle_t *ifofile,
                                  vts_attributes_t *vts_attributes, 
                                  unsigned int offset);
#define ifoRead_VTS_ATTRIBUTES(a,b,c) ___ifoRead_VTS_ATTRIBUTES(a,b,c)
static int ___ifoRead_C_ADT_internal(ifo_handle_t *ifofile, c_adt_t *c_adt,
                                  unsigned int sector);
#define ifoRead_C_ADT_internal(a,b,c) ___ifoRead_C_ADT_internal(a,b,c)
static int ___ifoRead_VOBU_ADMAP_internal(ifo_handle_t *ifofile,
                                       vobu_admap_t *vobu_admap, 
                                       unsigned int sector);
#define ifoRead_VOBU_ADMAP_internal(a,b,c) ___ifoRead_VOBU_ADMAP_internal(a,b,c)
static int ___ifoRead_PGCIT_internal(ifo_handle_t *ifofile, pgcit_t *pgcit,
                                  unsigned int offset);
#define ifoRead_PGCIT_internal(a,b,c) ___ifoRead_PGCIT_internal(a,b,c)
static void ___ifoFree_PGC(pgc_t *pgc);
#define ifoFree_PGC(a) ___ifoFree_PGC(a)
static void ___ifoFree_PGC_COMMAND_TBL(pgc_command_tbl_t *cmd_tbl);
#define ifoFree_PGC_COMMAND_TBL(a) ___ifoFree_PGC_COMMAND_TBL(a)
static void ___ifoFree_PGCIT_internal(pgcit_t *pgcit);
#define ifoFree_PGCIT_internal(a) ___ifoFree_PGCIT_internal(a)
static ifo_handle_t *___ifoOpen_File(ifo_handle_t *ifofile, int title,
                                  char *suffix);
#define ifoOpen_File(a,b,c) ___ifoOpen_File(a,b,c)
static ifo_handle_t *___ifoOpenVMGI_File(ifo_handle_t *ifofile, char *suffix);
#define ifoOpenVMGI_File(a,b) ___ifoOpenVMGI_File(a,b)
static ifo_handle_t *___ifoOpenVTSI_File(ifo_handle_t *ifofile, int title,
                                      char *suffix);
#define ifoOpenVTSI_File(a,b,c) ___ifoOpenVTSI_File(a,b,c)
#define DVDFileSeek_(a,b) ___DVDFileSeek_(a,b)
static int ___DVDFileSeek_( dvd_file_t *dvd_file, uint32_t offset ) {
  return (DVDFileSeek(dvd_file, (int)offset) == (int)offset);
}


ifo_handle_t *___ifoOpen(dvd_reader_t *dvd, int title) {
...
ifo_handle_t *___ifoOpenVMGI(dvd_reader_t *dvd) {
  ifo_handle_t *ifofile;
...
ifo_handle_t *___ifoOpenVTSI(dvd_reader_t *dvd, int title) {
  ifo_handle_t *ifofile;
...
static ifo_handle_t *___ifoOpenVTSI_File(ifo_handle_t* ifofile, int title, char *suffix) {
  if(!ifofile->file) {
    free(ifofile);
    return NULL;
  }
...
void ___ifoClose(ifo_handle_t *ifofile) {
  if(!ifofile)
    return;
  ...
  if(ifofile->vmgi_mat)
    freez(ifofile->vmgi_mat);

  if(ifofile->vtsi_mat)
    freez(ifofile->vtsi_mat);
...
int ___ifoRead_FP_PGC(ifo_handle_t *ifofile) {
...
static void ___ifoFree_PGC(pgc_t *pgc) {
  if(pgc) {
    ...
    if(pgc->cell_playback)
      freez(pgc->cell_playback);
    if(pgc->cell_position)
      freez(pgc->cell_position);
  }
}

void ___ifoFree_FP_PGC(ifo_handle_t *ifofile) {
  if(!ifofile)
    return;
...
int ___ifoRead_TT_SRPT(ifo_handle_t *ifofile) {
  tt_srpt_t *tt_srpt;
  int i, info_length;
...
void ___ifoFree_TT_SRPT(ifo_handle_t *ifofile) {
  if(!ifofile)
    return;
...
int ___ifoRead_VTS_PTT_SRPT(ifo_handle_t *ifofile) {
  ...
  if(!(DVDReadBytes(ifofile->file, vts_ptt_srpt, VTS_PTT_SRPT_SIZE))) {
    ...
    /*JMP free(vts_ptt_srpt);*/
    freez(ifofile->vts_ptt_srpt);
    return 0;
  }
...
void ___ifoFree_VTS_PTT_SRPT(ifo_handle_t *ifofile) {
  if(!ifofile)
    return;
...
int ___ifoRead_PTL_MAIT(ifo_handle_t *ifofile) {
  ptl_mait_t *ptl_mait;
  int info_length;
  unsigned int i, j;
...
void ___ifoFree_PTL_MAIT(ifo_handle_t *ifofile) {
  unsigned int i;
...
int ___ifoRead_VTS_TMAPT(ifo_handle_t *ifofile) {
...
void ___ifoFree_VTS_TMAPT(ifo_handle_t *ifofile) {
  unsigned int i;
...
int ___ifoRead_TITLE_C_ADT(ifo_handle_t *ifofile) {
...
int ___ifoRead_C_ADT(ifo_handle_t *ifofile) {
  unsigned int sector;
...
static int ___ifoRead_C_ADT_internal(ifo_handle_t *ifofile,
...
#define ifoFree_C_ADT_internal(a) ___ifoFree_C_ADT_internal(a)
static void ___ifoFree_C_ADT_internal(c_adt_t *c_adt) {
  if(c_adt) {
    freez(c_adt->cell_adr_table);
    free(c_adt);
  }
...
void ___ifoFree_C_ADT(ifo_handle_t *ifofile) {
  if(!ifofile)
    return;
...
void ___ifoFree_TITLE_C_ADT(ifo_handle_t *ifofile) {
  if(!ifofile)
    return;
...
int ___ifoRead_TITLE_VOBU_ADMAP(ifo_handle_t *ifofile) {
  if(!ifofile)
    return 0;
...
int ___ifoRead_VOBU_ADMAP(ifo_handle_t *ifofile) {
  unsigned int sector;
...
static int ___ifoRead_VOBU_ADMAP_internal(ifo_handle_t *ifofile,
  ...
  if(info_length && 
    ...
    freez(vobu_admap->vobu_start_sectors);
    return 0;
  }
...
#define ifoFree_VOBU_ADMAP_internal(a) ___ifoFree_VOBU_ADMAP_internal(a)
static void ___ifoFree_VOBU_ADMAP_internal(vobu_admap_t *vobu_admap) {
  if(vobu_admap) {
    freez(vobu_admap->vobu_start_sectors);
    free(vobu_admap);
  }
...
void ___ifoFree_VOBU_ADMAP(ifo_handle_t *ifofile) {
  if(!ifofile)
    return;
...
void ___ifoFree_TITLE_VOBU_ADMAP(ifo_handle_t *ifofile) {
  if(!ifofile)
    return;
...
int ___ifoRead_PGCIT(ifo_handle_t *ifofile) {
...
static void ___ifoFree_PGCIT_internal(pgcit_t *pgcit) {
  if(pgcit) {
    ...
    freez(pgcit->pgci_srp);
  }
}

void ___ifoFree_PGCIT(ifo_handle_t *ifofile) {
  if(!ifofile)
    return;
...
int ___ifoRead_PGCI_UT(ifo_handle_t *ifofile) {
  pgci_ut_t *pgci_ut;
  unsigned int sector;
  unsigned int i;  
  int info_length;
...
void ___ifoFree_PGCI_UT(ifo_handle_t *ifofile) {
  unsigned int i;
...
int ___ifoRead_VTS_ATRT(ifo_handle_t *ifofile) {
  vts_atrt_t *vts_atrt;
...
void ___ifoFree_VTS_ATRT(ifo_handle_t *ifofile) {
  if(!ifofile)
    return;
...
int ___ifoRead_TXTDT_MGI(ifo_handle_t *ifofile) {
...
void ___ifoFree_TXTDT_MGI(ifo_handle_t *ifofile) {
  if(!ifofile)
    return;
